...
using TechBer.ChuyenDoiSo.Test.Base.TestData;
...
namespace TechBer.ChuyenDoiSo.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="ChuyenDoiSoDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(ChuyenDoiSoDbContext context)
            {
        ...
        protected void UsingDbContext(Action<ChuyenDoiSoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ChuyenDoiSoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<ChuyenDoiSoDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<ChuyenDoiSoDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ChuyenDoiSoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ChuyenDoiSoDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ChuyenDoiSoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ChuyenDoiSoDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<ChuyenDoiSoDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<ChuyenDoiSoDbContext, Task<TResult>> func)
        {
            TResult result;
